...
| `UnicodeDecodeError: 'cp932' codec can't decode byte` (or `cp1252`) from a lesson script | Their copy predates 24 Aug 2026. Python on Windows read a UTF-8 file using the machine's ANSI code page. Nothing they typed, and it hits any Windows machine whose code page is not UTF-8 | `.\update.ps1`, say yes when it offers. Every file read in the app now names `encoding="utf-8"`. If git is not working, hand them a copy from your USB stick |
